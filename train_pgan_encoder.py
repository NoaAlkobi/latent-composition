...
def train(opt):
    ...
    batch_size = int(opt.batchSize/4)
    resolution=256
  ...
  # z datasets
    ...
    folder_to_save = 'training/pgan_'
    if opt.orig_code:
        folder_to_save += 'orig_'
    elif opt.only_mse:
        folder_to_save +='only_mse_'
    else:
    ...
    # folder_to_save = 'pgan_training_%s' % str(now.strftime("%d_%m_%Y_%H_%M_%S"))
    ...
    for epoch, epoch_loader in enumerate(pbar(
        ...
        for step, z_batch in enumerate(pbar(
            ...
            loss.backward()
            optimizerE.step()
            z_norm_total.append(z_batch.norm(2,dim=1).mean())
            encoded_norm_total.append(encoded.norm(2,dim=1).mean())
            ...
            if epoch % 20 ==0 and step % 20 == 0:
                ...
                if opt.orig_code:
                    ...
                    plt.legend(['total_loss','z_loss','mse loss','perceptual_loss'])
                elif opt.only_mse:
                    plt.plot(x,total_loss)
                else:
                    plt.plot(x,total_loss)
                    ...
                    plt.plot(x,mse_total_loss)
                    plt.plot(x,perceptual_total_loss)
                    plt.legend(['total_loss','mse loss','perceptual_loss'])
                ...
                plt.subplot(2,4,1)
                plt.imshow(convert_image_np(fake_im[0,:,:,:].reshape((1,3,out_shape,out_shape))))
                plt.title('real image (input)')
                plt.axis('off')
                plt.subplot(2,4,5)
                plt.imshow(convert_image_np(regenerated[0,:,:,:].detach().reshape((1,3,out_shape,out_shape))))
                plt.title('generated image G(z)')
                plt.axis('off')
                plt.subplot(2,4,2)
                plt.imshow(convert_image_np(fake_im[1,:,:,:].reshape((1,3,out_shape,out_shape))))
                plt.axis('off')
                plt.subplot(2,4,6)
                plt.imshow(convert_image_np(regenerated[1,:,:,:].detach().reshape((1,3,out_shape,out_shape))))
                plt.axis('off')
                plt.subplot(2,4,3)
                plt.imshow(convert_image_np(fake_im[2,:,:,:].reshape((1,3,out_shape,out_shape))))
                plt.axis('off')
                plt.subplot(2,4,7)
                plt.imshow(convert_image_np(regenerated[2,:,:,:].detach().reshape((1,3,out_shape,out_shape))))
                plt.axis('off')
                plt.subplot(2, 4, 4)
                plt.imshow(convert_image_np(fake_im[3, :, :, :].reshape((1, 3, out_shape, out_shape))))
                plt.axis('off')
                plt.subplot(2, 4, 8)
                plt.imshow(convert_image_np(regenerated[3, :, :, :].detach().reshape((1, 3, out_shape, out_shape))))
                plt.axis('off')
