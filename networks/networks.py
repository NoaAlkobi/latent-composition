...
def define_nets(nettype, domain, use_RGBM=True, use_VAE=False,
                ckpt_path='pretrained', load_encoder=True, device='cuda',resolution=256):
    ...
    if nettype == 'proggan':
        ...
        return BigGANNets(domain, resolution, use_RGBM, use_VAE, ckpt_path, load_encoder, device)
    else:
...
class BigGANNets(Nets):
    def __init__(self, domain,resolution, use_RGBM=True, use_VAE=False,
                 ckpt_path='pretrained', load_encoder=True, device='cuda'):
        from . import biggan

        setting = {'outdim': resolution, 'nz': 120, 'resnet_depth': 18}
        # path = r"../../deep_generative_prior/experiments/examples_fixed_G/res_%d" % (resolution)
        config = torch.load(r"config_res_%d.pth" % (resolution))
        self.generator = biggan.Generator(**config).to(device)
        # path = r"" % resolution
        self.generator.load_state_dict(torch.load(r"G_BigGAN_%d.pth" % (resolution)),strict=False)
        self.setting = setting
        self.device = device
        self.use_RGBM = use_RGBM
        self.use_VAE = use_VAE
    ...
    def sample_zs(self, n=100, seed=1, device=None):
        result = zdataset.z_sample_for_model(self.generator, n,
                                             seed,GAN='BigGAN').to(self.device)
        if device is None:
            result = result.to(self.device)
        else:
            result = result.to(device)
