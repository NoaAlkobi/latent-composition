    ...
    def __init__(self,
        ...
        if self.G_param == 'SN':
            ...
            self.which_linear = functools.partial(
                layers.SNLinear, num_svs=num_G_SVs, num_itrs=num_G_SV_itrs, eps=self.SN_eps)
        else:
        ...
        self.shared = (
            self.which_embedding(n_classes, self.shared_dim) if G_shared else layers.identity())
    ...
    def __init__(self,
        ...
        if self.D_param == 'SN':
            self.which_conv = functools.partial(
                ...
                eps=self.SN_eps)
            self.which_linear = functools.partial(
                layers.SNLinear, num_svs=num_D_SVs, num_itrs=num_D_SV_itrs, eps=self.SN_eps)
            self.which_embedding = functools.partial(
                layers.SNEmbedding, num_svs=num_D_SVs, num_itrs=num_D_SV_itrs, eps=self.SN_eps)
        else:
            self.which_conv = functools.partial(nn.Conv2d, kernel_size=3, padding=1)
            self.which_linear = nn.Linear
            self.which_embedding = nn.Embedding
...
def load_BigGAN_encoder(domain, nz=128, outdim=256, use_RGBM=True, use_VAE=False,
